...
4. В `/status` строка «Источники: все в порядке».
...
| `/status`: «ошибка оценки» | ключ, модель и лимиты LLM-провайдера; события с причиной `rank_invalid_output` модель не смогла оценить |
| В журнале `x results truncated by max_pages_per_query` | запрос слишком широкий: сузить его или повысить `max_pages_per_query` |
...
| `healthy`, но бот не реагирует на кнопки | ошибки `aiogram` в журнале: отозван токен или бот запущен во втором месте |
