...
Multi-source foundation (реестр источников, пакетная загрузка, метрики, BudgetGuard, retention, план GDELT + RSS) описан в [MULTI_SOURCE.md](MULTI_SOURCE.md). X и LLM — платные функции, по умолчанию выключены.
...
src/qmemo_radar/
  ...
  application/
    ...
    budget.py             BudgetGuard: платный вызов только после резерва в cost_ledger
    collection.py         MultiSourceCollector: источники параллельно, сбой одного изолирован
...
2. Collector получает checkpoints и по каждому источнику делает recent search: первый раз с `start_time` на 60 минут назад, дальше с `since_id`. Если `since_id` старше 6 дней (источник долго молчал, а recent search покрывает только 7 дней), снова используется `start_time`. Если после `max_pages_per_query` ещё остались страницы, более старые публикации пропускаются и в журнал пишется предупреждение `truncated`.
...
4. Элементы каждого источника пачками по 500 нормализуются, проходят фильтр (старое, заблокированные авторы, запрещённые слова, пустые публикации) и точную дедупликацию (id, URL, тот же текст), затем вставляются одним commit на пачку; отброшенное сохраняется как `FILTERED_OUT`. Checkpoint источника двигается только после записи всех его пачек. Метрики пишутся в `pipeline_metrics`.
5. Перед ранжированием первые 100 `DISCOVERED` проверяются повторно: ручные ссылки (им не страшен возраст), события старых версий и кандидаты, устаревшие в ожидании.
6. Если платный LLM выключен, ранжирование пропускается: события ждут или устаревают.
7. Ranker оценивает пачки до 10 событий. Невалидный ответ получает одну попытку исправления. Если пачка всё равно не оценена, её события оцениваются по одному, чтобы одна «отравленная» публикация не блокировала соседей; публикация, которая и одна даёт невалидный ответ, получает `FILTERED_OUT` с причиной `rank_invalid_output` и никогда не показывается. Если LLM недоступна (сеть, 5xx, 429), события остаются `DISCOVERED`, оценка в этом цикле прекращается и повторяется в следующем.
...
- `pipeline_metrics` — поток по источникам за каждый запуск;
- `cost_ledger` — резервы и оценки стоимости платных вызовов;
...
4. снимает webhook бота (long polling с ним не работает) и запускает один Telegram poller (только `message` и `callback_query`) и планировщик;
5. по SIGTERM или SIGINT сначала останавливает polling и до 20 секунд ждёт уже начатые обработчики (черновик, одобрение, `/run`), затем отменяет планировщик и закрывает HTTP-клиенты X, LLM и сессию бота. Прерванный сбор при следующем старте получает `FAILED` с причиной `interrupted`.
...
Healthcheck проверяет heartbeat планировщика. Он не обнаруживает зависший Telegram poller (aiogram бесконечно повторяет `getUpdates`, например при отозванном токене или втором экземпляре бота) — такие ошибки видны в журнале.
...
| M2 multi-source foundation | готово |
| M3 GDELT GQG + RSS | план в MULTI_SOURCE.md |
