...
- **Изоляция.** `MultiSourceCollector` запускает коллекторы параллельно. Исключение одного превращается в `SourceFetch(error_code="collector_failed:<тип>")`; остальные работают дальше.
...
- **Без дублей.** Повторный элемент распознаётся до вставки по `(source, external_id)` и `(source, url)`, внутри пачки — по множеству; уникальные индексы остаются последним уровнем защиты. Тот же текст из другого источника сохраняется `FILTERED_OUT` с `filter_reason=duplicate_content` и `duplicate_of_event_id` — это будущий сигнал «цитату повторили N источников».
...
- Резерв записывается до запроса внутри `BEGIN IMMEDIATE`: параллельные задачи и даже второй процесс ждут блокировку записи, поэтому не могут вместе превысить предел (тест: 20 параллельных вызовов по $0,30 при пределе $1 — проходят ровно 3).
- Деньги хранятся целыми микродолларами, округление вверх.
- X: страница резервирует худший случай (100 постов × $0,005 + 100 авторов × $0,010 = $1,50), после ответа сумма уменьшается до прочитанного. Цены — [docs.x.com pricing](https://docs.x.com/x-api/getting-started/pricing), проверено 2026-09-17; оплачиваются ли авторы из `includes`, не указано, поэтому они считаются платными.
...
Существующие колонки `radar_events.filter_reason` и `duplicate_of_event_id` (есть с миграции 001) теперь заполняются при вставке. Новые значения `SourceType` миграции не требуют: `source` — TEXT без CHECK.
